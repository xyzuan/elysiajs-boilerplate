...
import skUsahaSchema from "./sk-usaha.schema";

export const SkUsahaController = createElysia({
  prefix: "usaha",
})
  ...
  .patch(
    ...
    async ({ params: { id }, user, body }) => {
      const existingRecord = await prismaClient.user_sk.findUnique({
        where: { id, sk_type: SKType.USAHA, user_id: user.id },
        include: {
          user_approvers: true,
        },
      });
      ...
      if (!existingRecord) {
        throw new NotFoundException("SK Usaha record not found");
      }
