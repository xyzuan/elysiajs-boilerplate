...
import skDispensasiSchema from "./sk-dispensasi.schema";

export const SkDispensasiController = createElysia({
  prefix: "dispensasi",
})
  .use((app: Elysia) => rbac(app, "VIEW_SK"))
  .use(skDispensasiSchema)
  ...
  .patch(
    ...
    async ({ params: { id }, user, body }) => {
      const existingRecord = await prismaClient.user_sk.findUnique({
        where: { id, sk_type: SKType.DISPENSASI, user_id: user.id },
        include: {
          user_approvers: true,
        },
      });
      ...
      if (!existingRecord) {
        throw new NotFoundException("SK Dispensasi record not found");
      }
