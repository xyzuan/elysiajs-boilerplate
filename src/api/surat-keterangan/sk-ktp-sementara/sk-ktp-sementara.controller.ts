...
import skKtpSementaraSchema from "./sk-ktp-sementara.schema";

export const SkKtpSementaraController = createElysia({
  prefix: "ktp-sementara",
})
  .use((app: Elysia) => rbac(app, "VIEW_SK"))
  .use(skKtpSementaraSchema)
  ...
  .patch(
    ...
    async ({ params: { id }, user, body }) => {
      const existingRecord = await prismaClient.user_sk.findUnique({
        where: { id, sk_type: SKType.KTP_SEMENTARA, user_id: user.id },
        include: {
          user_approvers: true,
        },
      });
      ...
      if (!existingRecord) {
        throw new NotFoundException("SK KTP Sementara record not found");
      }
