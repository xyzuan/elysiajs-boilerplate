...
import skKelahiranSchema from "./sk-kelahiran.schema";

export const SkKelahiranController = createElysia({
  prefix: "kelahiran",
})
  .use((app: Elysia) => rbac(app, "VIEW_SK"))
  .use(skKelahiranSchema)
  ...
  .patch(
    ...
    async ({ params: { id }, user, body }) => {
      const existingRecord = await prismaClient.user_sk.findUnique({
        where: { id, sk_type: SKType.KELAHIRAN, user_id: user.id },
        include: {
          user_approvers: true,
        },
      });
      ...
      if (!existingRecord) {
        throw new NotFoundException("SK Kelahiran record not found");
      }
