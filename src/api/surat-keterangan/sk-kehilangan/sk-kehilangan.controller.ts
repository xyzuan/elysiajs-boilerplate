...
import skKehilanganSchema from "./sk-kehilangan.schema";

export const SkKehilanganController = createElysia({
  prefix: "kehilangan",
})
  .use((app: Elysia) => rbac(app, "VIEW_SK"))
  .use(skKehilanganSchema)
  ...
  .patch(
    ...
    async ({ params: { id }, user, body }) => {
      const existingRecord = await prismaClient.user_sk.findUnique({
        where: { id, sk_type: SKType.KEHILANGAN, user_id: user.id },
        include: {
          user_approvers: true,
        },
      });
      ...
      if (!existingRecord) {
        throw new NotFoundException("SK Kehilangan record not found");
      }
